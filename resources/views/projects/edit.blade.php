...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0 fw-bold">Edit Project</h1>
                    ...
                    <form action="{{ route('projects.update', $project) }}" method="POST">
                        ...
                        <div class="row mb-3">
                                ...
                                <select class="form-select" id="customer_id" name="customer_id">
                                    ...
                                    @isset($clients)
                                        @foreach($clients as $c)
                                            <option value="{{ $c->id }}" @selected(old('customer_id', $project->customer_id)==$c->id)>{{ $c->name }} ({{ $c->email }})</option>
                                        @endforeach
                                    @endisset
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <input type="number" class="form-control" id="order_quantity" name="order_quantity" value="{{ old('order_quantity', $project->order_quantity) }}" min="0">
                            </div>
                            <div class="col-md-3">
                                <label for="due_date" class="form-label fw-bold">Due Date</label>
                                <input type="date" class="form-control" id="due_date" name="due_date" value="{{ old('due_date', optional($project->due_date)->format('Y-m-d')) }}">
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="production_status" name="production_status">
                                    @foreach(['draft','scheduled','in_progress','blocked','completed'] as $s)
                                        <option value="{{ $s }}" @selected(old('production_status', $project->production_status)==$s)>{{ ucfirst(str_replace('_',' ',$s)) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="bpom_registration_id" name="bpom_registration_id">
                                    ...
                                    @isset($bpoms)
                                        @foreach($bpoms as $b)
                                            <option value="{{ $b->id }}" @selected(old('bpom_registration_id', $project->bpom_registration_id)==$b->id)>
